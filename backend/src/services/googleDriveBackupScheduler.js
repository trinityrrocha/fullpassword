...
const { getRawSettings } = require('./googleDriveBackupSettingsService');
const { runBackup } = require('./googleDriveBackupService');
...
const getNextExecutionAt = (settings, from = new Date()) => {
  if (!settings?.enabled || !settings?.schedule_enabled) return null;
...
const executeSchedulerTick = async () => {
  ...
  try {
    ...
    safeLogError('Falha sanitizada no agendador de backup Google Drive.', {
      name: 'GoogleDriveSchedulerError'
    }, { includeStack: false });
    await recordAuditEvent({
      action: 'google_drive_backup_failed',
      status: 'failed',
      metadata: { trigger_type: 'scheduled', reason: error?.code || 'operation_failed' },
      queryable: client
    }).catch(() => {});
  } finally {
...
const startGoogleDriveBackupScheduler = () => {
