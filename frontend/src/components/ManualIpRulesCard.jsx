  ...
  return (
    <SettingsAccordionCard title="Blacklist / Whitelist Manual" icon={<ListPlus className="w-5 h-5 mr-2 text-indigo-500" />}>
      <div className="space-y-6">
        ...
        <div className="overflow-x-auto rounded-lg border border-slate-200"><table className="min-w-full text-sm"><thead className="bg-slate-50"><tr>{['IP/CIDR', 'Tipo', 'Motivo', 'Criado por', 'Criado em', 'Expira em', 'Status', 'Ação'].map((label) => <th key={label} className="px-3 py-3 text-left">{label}</th>)}</tr></thead><tbody>{rules.length ? rules.map((rule) => <tr key={rule.id} className="border-t"><td className="px-3 py-3 font-mono">{rule.ip_address}</td><td className="px-3 py-3"><span className={`rounded-full px-2 py-1 text-xs ${ruleStyles[rule.rule_type]}`}>{ruleLabels[rule.rule_type]}</span></td><td className="px-3 py-3">{rule.reason || '-'}</td><td className="px-3 py-3">{rule.created_by_email || '-'}</td><td className="px-3 py-3 whitespace-nowrap">{new Date(rule.created_at).toLocaleString('pt-BR')}</td><td className="px-3 py-3 whitespace-nowrap">{rule.expires_at ? new Date(rule.expires_at).toLocaleString('pt-BR') : '-'}</td><td className="px-3 py-3">{rule.is_active ? 'Ativa' : 'Inativa'}</td><td className="px-3 py-3"><button type="button" onClick={() => deactivateRule(rule.id)} className="text-red-600">Desativar regra</button></td></tr>) : <tr><td colSpan={8} className="px-3 py-6 text-center text-slate-500">{isLoading ? 'Carregando regras...' : 'Nenhuma regra encontrada.'}</td></tr>}</tbody></table></div>
      </div>
