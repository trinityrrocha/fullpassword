...
const serverConnectionFields = read('src/components/ServerConnectionFields.jsx');
...
for (const [name, source] of managerSources) {
  ...
  const connectionSection = name === 'Dispositivos' ? source.slice(connectionStart, connectionEnd) : serverConnectionFields;
  assert.doesNotMatch(connectionSection, name === 'Dispositivos' ? /overflow-x-auto|overflow-x-scroll|min-w-\[/ : /overflow-x-auto|overflow-x-scroll/, `Conexões de ${name} não devem criar rolagem horizontal`);
