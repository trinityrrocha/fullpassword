...
assert.match(layout, /api\.post\('\/system\/security-notifications\/mark-seen'/);
assert.match(layout, /<SecurityNotificationsMenu/);
assert.match(layout, /seen_through: seenThrough/);
assert.match(layout, /setNotifications\(\(current\) => \(\{ \.\.\.current, unread_count: 0 \}\)\)/);
assert.match(layout, /window\.setInterval\(refreshNotifications, 60000\)/);
...
assert.match(layout, /setIsNotificationsOpen\(true\)[\s\S]*api\.get\('\/system\/security-notifications'\)/);
